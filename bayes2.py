...
def spamTest():
	...
	p0V,p1V,pSpam = trainNB0(array(tainMat),array(trainClasses))
	errorCount = 0
...
def localWords(feed1,feed0):
	...
	for i in range(minLen):
		wordList = textParse(feed1['tnries'][i]['summary'])
		docList.append(wordList)
		fullText.extend(wordList)
		classList.append(1)
		wordList = textParse(feed0['tnries'][i]['summary'])
		docList.append(wordList)
		fullText.extend(wordList)
		classList.append(0)
	vocabList = createVocabLIst(docList)
	...
	trainingSet = range(2*mnLen); testSet = []
	for i in range(20):
		randIndex = int(random.uniform(0,len(trainingSet)))
		testSet.append(trainingSet[randIndex])
		del(trainingSet[randIndex])
